...
const CreateAccountForm = ({ title }) => {
  ...
  useEffect(() => {
    const validateForm = () => {
      const { accountName, currency, accountType, term, withdrawalAccountId } = values;
      const isValid = accountName && currency && accountType && 
                      (accountType !== 'Deposit' || (term && withdrawalAccountId));
